  ...
  (let ((scale (coerce scale 'function)))
    ...
    (cl-gdal::with-gdal-file (handle file)
      (let* ((data (cl-gdal::gdal-read-all-data (cl-gdal::gdal-get-raster-band handle 1)))
             (x-size 3348);;3348);; input data is 2576 ;; dlon is 0.02 degrees which is 1.4 km at the south of Canada and 2.2 km at the equator
             (y-size 2162);; 2162);; input data is 1456 ;; dlat is 0.03 degrees, which is 3.3 km km at the northern most part of Canada, and 2.2 km at the south of Canada.
             (ulx (float *ulx* 0d0))
             (uly (float *uly* 0d0))
             (dlon (float (abs (/ (- *lrx* ulx) x-size)) 0d0))
             (dlat (float (abs (/ (- *lry* uly) x-size)) 0d0))
	...
	(declare (type (simple-array double-float (* *)) data))
        (cached-transforms:with-cached-transform (xpixelref ypixelref ulx uly dlon dlat x-size y-size data-xsize data-ysize source-spatial-reference source-geo-transform)
                ...
                (let ((tile-iterator (alexandria:ensure-function (tile-iterator))))
                  ...
                  (iter 
                    ...
                    (destructuring-bind (x-pixel-min y-pixel-min x-pixel-max y-pixel-max)
                        (mapcar #'round
                                (tile-bounds-to-pixel-bounds tile (lambda (lon lat)
