...
(defun generate-lon-lat-to-pixel-transform-array (x-size y-size source-spatial-reference source-geo-transform)
  ...
  "Returns (lambda (lon-array lat-array) -> pixelx-array, pixely-array)
   Remember pixel 0 0 is the upper left of the image... so the bottom
   right of a 100x100 pixel image is actually at pixel 100x100 (the
   bottom right of the last pixel at (99,99))."
  (let* ((x-size-df (float x-size 0d0))
	 ...
	 (uly (aref source-geo-transform 3))
         (source-width (- lrx ulx))
         (source-height (- lry uly)))
    ...
    (labels ((from-source-geo-ref-to-source-pixel (xarr yarr)
               ...
               (values
                (map '(simple-array (unsigned-byte 16) (*)) (lambda (x)
                                                              ...
                                                              (let ((x (round (the (double-float -1d9 1d9) (* x-size-df (/ (- x ulx) source-width))))))
                                                                (if (or (< x 0) (>= x x-size)) +out-of-source-range+ x))) xarr)
                (map '(simple-array (unsigned-byte 16) (*)) (lambda (y)
                                                              (declare (type double-float y))
                                                              (let ((y (round (the (double-float -1d9 1d9) (* y-size-df (/ (- y uly) source-height))))))
                                                                (declare (type (signed-byte 32) y))
                                                                (if (or (< y 0) (>= y y-size)) +out-of-source-range+ y))) yarr)))
	     (from-google-geo-ref-to-source-geo-ref (lons lats)
               ...
               (let ((res (multiple-value-list (time (transform-point-array itransform lons lats)))))
                 (time (apply #'from-source-geo-ref-to-source-pixel res)))))
